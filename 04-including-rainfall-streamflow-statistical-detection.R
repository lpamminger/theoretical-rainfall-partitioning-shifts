...
all_combinations_parameter_multi_rainfall <- as.matrix(expand.grid(streamflow_parameters_index, 
...
ggsave(paste0("./Graphs/combined_residual_detection_", str_remove_all(Sys.Date(), "-"), ".pdf"),
...
write_csv(all_tests, file = "test.csv")
...
ggplotting_p_value_facet <- function(summary_p_values){
  ...
  ggplot_results <- plotting_summary_p_values |> 
    ...
    labs(x = "Years of data post change",
         y = "P-value significance of residuals") +
    theme_bw() +
    facet_grid(parameter ~ multiplier,
               labeller = labeller(
                 parameter = parameter_labs,
...
ggsave(paste0("./Graphs/residual_detection_test_ks_", str_remove_all(Sys.Date(), "-"), ".pdf"),
...
ggsave(paste0("./Graphs/residual_detection_test_fligner_", str_remove_all(Sys.Date(), "-"), ".pdf"),
