...
generated_rainfall <- modified_stochastic_rainfall_generator(
  parameter_vector = rainfall_temperate_parameters |> pull(control),
...
control_parameters <- partitioning_temperate_parameters |> pull(control)


## Make up change parameters ===================================================
### This will likely be a mix of small_multipliers and large_multipliers

multipliers_for_control_parameters <- partitioning_temperate_parameters |> 
  pull(small_multiplier)

### Autocorrelation, Standard deviation and skewness little change using the small_multipliers
### Replace these parameters with large_multiplier

multipliers_for_control_parameters[3:5] <- partitioning_temperate_parameters$large_multiplier[3:5]
...
boxcox_streamflow_list <- map(
  .x = parameter_list[-1], # exclude control
...
# Get in tidy form
## I do not know how to pivot longer into this format --> parameter|control_or_change|rainfall|boxcox_mean_streamflow|boxcox_streamflow|streamflow
...
x <- tidy_boxcox_streamflow |>
  filter(parameter == "intercept") |> # must be intercept
...
make_annotation_inset <- function(parameter, data) {
  ...
  annotation <- annotation_custom2(
    ...
    xmin = 410, # these are hard coded - CHANGE MANUALLY
    xmax = 1000,
    ymin = 13.5,
    ymax = 24
  )
