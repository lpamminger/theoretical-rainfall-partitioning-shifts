...
## Use the same as 03 file
multipliers_for_control_rainfall_parameters <- rainfall_temperate_parameters |> pull(small_multiplier)

multipliers_for_control_rainfall_parameters[3:4] <- rainfall_temperate_parameters$large_multiplier[3:4]


change_rainfall_parameters <- imap(
  .x = multipliers_for_control_rainfall_parameters,
...
replicates_rainfall_runoff <- function(replicate_number, change_rainfall_parameters, control_rainfall_parameters, control_streamflow_parameters, skip) {
  ...
  names_vector <- rainfall_temperate_parameters |> pull(metric)
...
hist_plot <- pivot_for_plotting |>
  ...
  mutate(
    parameter_changed = factor(
      # factor used to set order of parameter_changed
      parameter_changed,
